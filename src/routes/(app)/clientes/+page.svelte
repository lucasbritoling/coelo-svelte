<script lang="ts">
    ...
    function handleSearch(e: Event) {
        ...
        searchTimeout = setTimeout(() => {
            ...
            goto(newUrl.search, { keepFocus: true, replaceState: true, noScroll: true })
...
<div class="flex h-full flex-col bg-white">
    ...
    <header class="flex flex-col gap-4 px-6 pt-8 pb-4">
        ...
        <div class="relative">
            <Search class="absolute top-3 left-3.5 size-4 {isSearching ? 'animate-pulse text-blue-500' : 'text-zinc-400'}" />
            <Input
                type="search"
    ...
    <div class="flex-1 space-y-2 overflow-y-auto px-4 pb-32">
        {#each filteredCustomers as customer (customer.id)}
            ...
            >
                <div class="flex size-11 shrink-0 items-center justify-center rounded-full bg-zinc-100 text-sm font-bold text-zinc-500">
...
<Dialog.Root bind:open>
    <Dialog.Content class="flex max-h-[90dvh] w-[95vw] flex-col gap-0 overflow-hidden rounded-[32px] p-0 sm:max-w-[400px]">
        <Dialog.Header class="px-6 py-6 text-left">
            <Dialog.Title class="text-xl font-bold">{formState.id ? 'Editar' : 'Novo Cliente'}</Dialog.Title>
        </Dialog.Header>
        ...
        >
            <div class="space-y-6 px-6 pb-8">
                ...
                <div class="grid gap-2">
                    <Label class="text-xs font-bold text-zinc-400 uppercase tracking-widest">Nome</Label>
                    <Input name="name" bind:value={formState.name} required class="h-12 rounded-2xl bg-zinc-50 border-none" />
                </div>
                <div class="grid gap-2">
                    <Label class="text-xs font-bold text-zinc-400 uppercase tracking-widest">WhatsApp</Label>
                    <Input
                        class="h-12 rounded-2xl bg-zinc-50 border-none font-mono"
            ...
            <div class="flex gap-3 border-t p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
                ...
                {/if}
                <Button type="submit" disabled={isLoading} class="h-12 flex-[2] rounded-2xl bg-zinc-900 text-white">
                    {isLoading ? '...' : 'Salvar'}
                </Button>
            </div>
        </form>
