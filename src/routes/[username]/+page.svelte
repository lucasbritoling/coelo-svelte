...
<div class="mx-auto p-6 {data.singleService ? 'max-w-sm' : 'max-w-sm lg:max-w-5xl'}">
    ...
    {#if isSuccess && bookingSnapshot}
        ...
        <div class="mx-auto max-w-md animate-in fade-in zoom-in-95 duration-500">
            <Card.Root>
                <Card.Content class="flex flex-col items-center justify-center p-8 text-center">
                    <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 shadow-sm">
                    ...
                    <div class="mt-8 w-full space-y-3 rounded-xl border bg-muted/30 p-4 text-left">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex flex-col">
                                <span class="text-[10px] tracking-wider text-muted-foreground uppercase">Data e Hora</span>
                    ...
                    <p class="mt-8 text-[10px] text-muted-foreground">
    ...
    {:else if !data.hasActiveServices}
        ...
        <Card.Root class="mx-auto max-w-md border-dashed">
            <Card.Content class="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p class="mt-2 text-sm text-muted-foreground">Este profissional não possui serviços ativos.</p>
        ...
        <div class="grid gap-8 lg:grid-cols-{data.singleService ? '2' : '3'}">
            {#if !data.singleService}
                ...
                <Card.Root class="lg:col-span-1">
                    ...
                    <Card.Content>
                        <RadioGroup.Root value={data.selectedServiceId} onValueChange={(id) => updateSelection({ serviceId: id })}>
                            {#each services as service (service.id)}
                                <div class="flex items-center space-x-2 rounded-lg border p-3 transition-colors hover:bg-muted">
                ...
                <Card.Header>
                    <div class="flex h-8 items-center justify-between">
                        <div>
                            ...
                            <Card.Description>
                                {#if !data.selectedServiceId}Selecione um serviço primeiro{:else if !data.selectedDate}Selecione uma data primeiro{/if}
                            </Card.Description>
                        </div>
                        {#if !isConfirming && selectedSlot}
                            <Button size="sm" class="h-7.5 w-20.5" onclick={() => (isConfirming = true)}>Avançar</Button>
                        {/if}
                    </div>
                </Card.Header>
